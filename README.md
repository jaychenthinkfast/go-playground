...
一个现代化的在线 Go 代码运行和分享平台。支持多个 Go 版本（1.22、1.23、1.24、1.25），提供代码运行、格式化和分享功能。

## 功能特性

- 🚀 支持多个 Go 版本（1.22、1.23、1.24、1.25）
- 💻 在线编辑和运行 Go 代码
- 🎨 代码格式化
- 📤 代码分享功能（带有过期选项）
- 🔄 实时运行结果
...
### 后端
- Go (支持 1.22、1.23、1.24、1.25 版本)
- Gin Web 框架
- MongoDB 4.4 数据库（持久化存储，默认 ./data/mongo）
  - 存储分享的代码内容
...
> 注意：数据持久化与缓存默认挂载到当前目录 `./data/`（如 `./data/mongo`、`./data/share-service-go-mod-cache`、`./data/backend-go*-mod-cache`）。
...
1. **MongoDB 4.4**：作为主要存储，永久保存所有分享数据
...
2. **版本支持**：
   - 支持多种Go版本格式：`go1.25`、`1.25`、`go1.25.0`等
   - 自动标准化版本格式，确保正确路由
